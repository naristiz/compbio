...
```{r}
p2 <- ggplot(fake_data, aes(forest, fruit)) +
  geom_point(aes(colour = factor(category), shape = factor(site))) +
   labs(
    x = "Coffee fruit size (mg)",
...
Now by separating by categorical data on grids and colors
```{r}
...
p6 <- ggplot(data=fake_data, aes(x=forest,y=fruit), color=category) + 
  ...
  geom_line(aes(y = fruit), size = 0.1) +
...
```{r}
mytheme <- theme(axis.title.x = element_text(size=20, color = "black", face = "bold"), 
...
```{r}
favorite <- ggplot(data=fake_data, aes(x=forest,y=fruit), col=site, color=category) + 
  facet_grid(~site) +
  geom_violin(aes(y=fruit, color=category), size=0.5) +
  ylab("Coffee berry size (mg)") +
  xlab("Forest cover in 1 km radius (%)") + 
  theme_test() + 
  mytheme + 
  #scale_fill_discrete(name = "Experimental sites")
